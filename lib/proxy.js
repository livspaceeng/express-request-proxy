...
module.exports = function(options, errorhandler) {
  ...
  function makeApiCall(req, res, next) {
    ...
    try{
      ...
      console.log('__proxy_logger__error_in_making_request', JSON.stringify(error))
      next(Error.http(599, 'Request could not be made'));
    }


    // Abort api request if client close its connection
    req.on('close', function() {
      console.log('__proxy_logger__client_close_connection')
    ...
    apiRequest.on('response', function(resp) {
      ...
      if (resp.statusCode >= 400) {
        console.log('__proxy_logger__api_errors', JSON.stringify(resp))
        debug('Received error %s from %s', resp.statusCode, apiRequestOptions.url);
        /**
         * If error handler is specified then we are calling error handler only to handle the errors instead of piping
         */
        if (errorhandler && typeof errorhandler === "function") {
          return errorhandler(resp, req, res, apiRequest);
        } else {
      ...
      try {
        ...
        console.log('__proxy_logger__transform_error', JSON.stringify(error))
        next(Error.http(599, 'Error in transformation'));
      }
  ...
  function unhandledApiError(err, next) {
    ...
    console.log('__proxy_logger__unhandled_API_error_', JSON.stringify(err))
